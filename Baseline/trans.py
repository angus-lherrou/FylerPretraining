...
class TransformerClassifier(nn.Module):
    ...
    def __init__(self, num_classes=2):
        ...
        self.linear = nn.Linear(
            in_features=cfg.getint("model", "emb_dim"), out_features=num_classes
        )
...
def make_data_loader(model_inputs, labels, batch_size, partition):
    """DataLoader objects for train or dev/test sets"""

    tensor_dataset = TensorDataset(model_inputs, torch.tensor(labels))
...
def fit(model, train_loader, val_loader, weights, n_epochs):
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=cfg.getfloat("model", "lr"))
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        print(
            "ep: %d, steps: %d, tr loss: %.3f, val loss: %.3f, val roc: %.3f"
...
def select_model():
    ...
    tr_texts, tr_labels = datareader.DirDataReader.read(
        os.path.join(base, cfg.get("data", "train")), label2int
    )
    ...
    print(
        "loaded %d training and %d validation samples" % (len(tr_texts), len(val_texts))
    )
    ...
    best_roc, optimal_epochs = fit(
        model, train_loader, val_loader, weights, cfg.getint("model", "n_epochs")
    )
...
def run_evaluation(optimal_epochs):
    ...
    test_texts, test_labels = datareader.DirDataReader.read(
        os.path.join(base, cfg.get("data", "test")), label2int
    )
    ...
    print("loaded %d training and %d test samples" % (len(tr_texts), len(test_texts)))
