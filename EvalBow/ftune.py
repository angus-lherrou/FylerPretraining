...
def get_model():
    ...
    model.classifier = torch.nn.Linear(
        in_features=config["hidden_units"], out_features=2
    )
...
def fit(model, train_loader, test_loader, weights, n_epochs):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        print(
            "ep: %d, steps: %d, tr loss: %.4f, val loss: %.4f, val roc: %.4f"
...
def eval_on_dev():
    ...
    train_loader = make_data_loader(
        x_train, torch.tensor(y_train), cfg.getint("model", "batch"), "train"
    )
    val_loader = make_data_loader(
        x_val, torch.tensor(y_val), cfg.getint("model", "batch"), "dev"
    )
    ...
    best_roc_auc, optimal_epochs = fit(
        model, train_loader, val_loader, weights, cfg.getint("model", "epochs")
    )
...
def eval_on_test(n_epochs):
    ...
    test_data_provider = DatasetProvider(
        os.path.join(base, cfg.get("data", "test")), cfg.get("data", "tokenizer_pickle")
    )
    ...
    test_loader = make_data_loader(
        x_test, torch.tensor(y_test), cfg.getint("model", "batch"), "dev"
    )
